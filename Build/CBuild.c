...
int main(int _argc, char **_argv)
{
    char *ptr;
    int i;
    /* These have to be in the main scope due to compiler limitations. */
        ...
        if(strcasecmp("do", linebuf) == 0)
        {
            ...
            if(do_level >= sizeof(int)*8)
            {
                printf("\n\n!!! %s error, line %d !!!\n"
                       ...
                       fname, curr_line, (unsigned)(sizeof(int)*8 - 1));
...
dir_write_check:
        ...
        * file types are silently ignored. */
        if(strcasecmp("compile", linebuf) == 0)
        {
            ...
            for(index = 0; index < strlen(sources); index++)
            {
                /* Ignore any superfluous whitespace. */
                if(isspace(sources[index]))
                {
                    if(was_space == 0)
                    {
                        was_space = 1;
