...
static UINT32
APU_LinearToLog (INT32 l)
{
  return (l < 0) ? (apu_lineartbl[-l] + 1) : apu_lineartbl[l];
}

static INT32
APU_LogToLinear (UINT32 l, UINT32 sft)
{
  INT32 ret;
  UINT32 ofs;
  sft += (l >> 1) >> APU_LOG_BITS;
  if (sft >= APU_LOG_LIN_BITS)
    return 0;
  ...
  for (i = 1; i < (1 << APU_LIN_BITS) + 1; i++)
    {
      ...
      ua =
	(UINT32) ((APU_LOG_LIN_BITS -
		   (log (a) / log (2))) * (1 << APU_LOG_BITS));
      apu_lineartbl[i] = ua << 1;
    }
}

static UINT32
APU_DivFix (UINT32 p1, UINT32 p2, UINT32 fix)
{
  UINT32 ret;
