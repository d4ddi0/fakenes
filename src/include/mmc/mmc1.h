...
static void mmc1_save_state(PACKFILE*, const int);
static void mmc1_load_state(PACKFILE*, const int);
...
static INLINE void mmc1_update_prg_bank_select(void)
{
   if(mmc1_register[0] & MMC1_PRG_BANK_SIZE_BIT) {
      /* 16k PRG banking? */
      const int mmc1_bank_number = mmc1_register[3] & 0x0F;
      ...
      /* No, 32k PRG banking */
      const int mmc1_bank_number = (mmc1_register[3] & 0x0F & ~1);
      mmc1_cpu_bank[0] = mmc1_bank_number;
...
void mmc1_update_256k_prg_banking(void)
{
   ...
   if(mmc1_register[0] & MMC1_USE_256K_SELECT_1_BIT) {
      mmc1_256k_bank_num = (mmc1_register[1] & MMC1_256K_SELECT_0_BIT) >> 4;
      if(mmc1_register[0] & MMC1_PRG_BANK_SIZE_BIT)
         /* Er...? */
         mmc1_256k_bank_num |= ((mmc1_register[2] & MMC1_256K_SELECT_1_BIT) >> 3);
   }
   else
      mmc1_256k_bank_num = (mmc1_register[1] & MMC1_256K_SELECT_0_BIT) ? 3 : 0;

   mmc1_update_prg_banking();
}

/* passed a register selection address, 0-3 */
static void mmc1_set_register_value(const unsigned address, const UINT8 value)
{
   ...
   switch (address) {
      case 0: {
         if(mmc1_register[0] & MMC1_MIRRORING_MODE_BIT)
            ...
            /* which way? */
            ppu_set_mirroring((mmc1_register[0] & MMC1_MIRRORING_ADDRESS_BIT) ?
         ...
         if(ROM_CHR_ROM_PAGES > 0) {
            if(mmc1_register[0] & MMC1_VROM_BANK_SIZE_BIT) {
               ...
               /* swap 4k of CHR-ROM */
               ppu_set_1k_pattern_table_vrom_page(0 << 10, mmc1_bank_number);
               ppu_set_1k_pattern_table_vrom_page(1 << 10, mmc1_bank_number + 1);
               ppu_set_1k_pattern_table_vrom_page(2 << 10, mmc1_bank_number + 2);
               ppu_set_1k_pattern_table_vrom_page(3 << 10, mmc1_bank_number + 3);

               mmc1_bank_number = mmc1_register[2] * 4;
                /* swap other 4k of CHR-ROM */
               ppu_set_1k_pattern_table_vrom_page(4 << 10, mmc1_bank_number);
               ppu_set_1k_pattern_table_vrom_page(5 << 10, mmc1_bank_number + 1);
               ppu_set_1k_pattern_table_vrom_page(6 << 10, mmc1_bank_number + 2);
               ppu_set_1k_pattern_table_vrom_page(7 << 10, mmc1_bank_number + 3);
            }
            else {
               /* No, 8k VROM mapping */
               const int mmc1_bank_number = (mmc1_register[1] & ~1) * 4;
               /* swap 8k of CHR-ROM */ //never called??
               ppu_set_1k_pattern_table_vrom_page(0 << 10, mmc1_bank_number);
               ppu_set_1k_pattern_table_vrom_page(1 << 10, mmc1_bank_number + 1);
               ppu_set_1k_pattern_table_vrom_page(2 << 10, mmc1_bank_number + 2);
               ppu_set_1k_pattern_table_vrom_page(3 << 10, mmc1_bank_number + 3);
               ppu_set_1k_pattern_table_vrom_page(4 << 10, mmc1_bank_number + 4);
               ppu_set_1k_pattern_table_vrom_page(5 << 10, mmc1_bank_number + 5);
               ppu_set_1k_pattern_table_vrom_page(6 << 10, mmc1_bank_number + 6);
               ppu_set_1k_pattern_table_vrom_page(7 << 10, mmc1_bank_number + 7);
            }
         }
      ...
      case 1: {
         ...
         if(ROM_CHR_ROM_PAGES > 0) {
            if(mmc1_register[0] & MMC1_VROM_BANK_SIZE_BIT) {
               /* 4k VROM mapping? */
               const int mmc1_bank_number = mmc1_register[1] * 4;
      ...
      case 2: {
         const int mmc1_bank_number = mmc1_register[2];
...
{
   const int mmc1_current_register = (address & 0x6000) >> 13;
   ...
   if(value & 0x01)
      mmc1_bit_stream |= (1 << mmc1_bit_counter); //neat idea, nester dude!

   mmc1_bit_counter++;
   if(mmc1_bit_counter < 5)
      return;
...
static void mmc1_save_state(PACKFILE* file, const int version)
{
   RT_ASSERT(file);
...
static void mmc1_load_state(PACKFILE* file, const int version)
{
   RT_ASSERT(file);
