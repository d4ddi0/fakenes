...
#define CPU__RAM_SIZE		(0xFFFF + 1)
#define CPU__RAM_PAGE_SIZE	1024
#define CPU__RAM_PAGE_MASK	(CPU_RAM_PAGE_SIZE - 1)
#define CPU__RAM_PAGES		(CPU__RAM_SIZE / CPU__RAM_PAGE_SIZE)

QUICK UINT8 cpu__fast_read(const UINT16 address)
{
   const int page = address / CPU__RAM_PAGE_SIZE;
   const UINT8* read = cpu__ram_read[page];
   return read[address & CPU__RAM_PAGE_MASK] + cpu__patch_ram[address];
}

QUICK void cpu__fast_write(const UINT16 address, const UINT8 data)
{
   const int page = address / CPU__RAM_PAGE_SIZE;
   UINT8* write = cpu__ram_write[page];
   write[address & CPU__RAM_PAGE_MASK] = data;  
}

QUICK UINT8 cpu__fast_ram_read(const UINT16 address)
{
   return cpu__ram[address] + cpu__patch_ram[address];
}

QUICK void cpu__fast_ram_write(const UINT16 address, const UINT8 data)
{
   cpu__ram[address] = data;
}
