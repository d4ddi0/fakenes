...
extern ustring ustring_from_c_string(const char* input, const size_type size);
extern ustring ustring_from_utf_string(const utf_string& input);
extern utf_data* ustring_to_data(const ustring& input, utf_data* output, const UNICODE_FORMAT format, size_type& size);
extern std::string ustring_to_string(const ustring& input);
extern char* ustring_to_c_string(const ustring& input, char* output, size_type& size);
